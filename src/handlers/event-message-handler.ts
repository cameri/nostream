...
import { getPubkeysFromTags } from "../utils/appUtils";
...
export class EventMessageHandler implements IMessageHandler {
  ...
  public async handleMessage(message: IncomingEventMessage): Promise<void> {
    ...
    if (!this.isAppCompliant(event)) {
      debug("event %s rejected: not app compliant", event.id);
      this.webSocket.emit(
        WebSocketAdapterEvent.Message,
        createCommandResult(event.id, false, "event is not app compliant")
      );
      return;
    }
  ...
  protected async isRateLimited(event: Event): Promise<boolean> {
    ...
    if (this.isPubkeyWhitelisted(event)) {
      return false;
    }
  ...
  /**
   * Determines if a given event is relevant to a bot-app.
   */
  protected isAppCompliant(event: Event): boolean {
    return this.isPubkeyWhitelisted(event) || this.isRecipientValid(event);
  }

  protected isPubkeyWhitelisted(event: Event): boolean {
    const { whitelists } = this.settings().limits?.event ?? {};
    if (
  ...
  protected isRecipientValid(event: Event): boolean {
    const { clientlist } = this.settings().limits?.event?.tags ?? {};
    const recipientPubkeys = getPubkeysFromTags(event);
    // Return true if any one of the recipients is in the clientlist.
    if (
      clientlist &&
      Array.isArray(clientlist) &&
      recipientPubkeys.some((recipient) => clientlist.includes(recipient))
    ) {
      return true;
    }
    return false;
  ...
  protected async isUserAdmitted(event: Event): Promise<string | undefined> {
    ...
    const isApplicableFee = (feeSchedule: FeeSchedule) =>
      ...
      !feeSchedule.whitelists?.event_kinds?.some(
        isEventKindOrRangeMatch(event)
      );

    const feeSchedules =
      currentSettings.payments?.feeSchedules?.admission?.filter(
        isApplicableFee
      );
