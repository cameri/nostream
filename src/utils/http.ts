...
export const getRemoteAddress = (request: IncomingMessage, settings: ISettings): string | undefined => {
  ...
  if ('network' in settings && 'remote_ip_header' in settings.network) {
    ...
    header = settings.network['remote_ip_header'] as string | undefined
  } else {
    header = settings.network?.remoteIpHeader
  }
