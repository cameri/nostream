...
When(
  ...
  async function (this: World<Record<string, any>>, from: string, to: string, limit: string) {
    const ws = this.parameters.clients[from] as WebSocket
...
When(
  ...
  async function (this: World<Record<string, any>>, name: string, author1: string, author2: string) {
    const ws = this.parameters.clients[name] as WebSocket
...
When(
  ...
  async function (this: World<Record<string, any>>, name: string, since: string, until: string) {
    const ws = this.parameters.clients[name] as WebSocket
...
When(
  ...
  async function (this: World<Record<string, any>>, name: string, tag: string, value: string) {
    const ws = this.parameters.clients[name] as WebSocket
...
Then(
  ...
  async function (name: string, author: string, content: string, createdAt: string) {
    const ws = this.parameters.clients[name] as WebSocket
    const subscription = this.parameters.subscriptions[name][this.parameters.subscriptions[name].length - 1]
    const receivedEvent = await waitForNextEvent(ws, subscription.name, content)
...
Then(/(\w+) receives (\d+) text_note events from (\w+)/, async function (name: string, count: string, author: string) {
  const ws = this.parameters.clients[name] as WebSocket
  const subscription = this.parameters.subscriptions[name][this.parameters.subscriptions[name].length - 1]
...
Then(
  /(\w+) receives (\d+) events from (\w+) and (\w+)/,
  async function (name: string, count: string, author1: string, author2: string) {
    const ws = this.parameters.clients[name] as WebSocket
    const subscription = this.parameters.subscriptions[name][this.parameters.subscriptions[name].length - 1]
    const events = await waitForEventCount(ws, subscription.name, Number(count), true)
