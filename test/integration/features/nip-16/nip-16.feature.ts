...
When(/^(\w+) sends a ephemeral_event_0 event with content "([^"]+)"$/, async function (name: string, content: string) {
  const ws = this.parameters.clients[name] as WebSocket
  const { pubkey, privkey } = this.parameters.identities[name]
...
Then(/(\w+) receives (\d+) ephemeral_event_0 events? and EOSE/, async function (name: string, count: string) {
  const ws = this.parameters.clients[name] as WebSocket
  const subscription = this.parameters.subscriptions[name][this.parameters.subscriptions[name].length - 1]
  const events = await waitForEventCount(ws, subscription.name, Number(count), true)
...
When(
  /^(\w+) sends two identically-timestamped replaceable_event_0 events where the second has a lower ID$/,
  async function (name: string) {
    ...
    const event1 = await createEvent(
      { pubkey, kind: 10000, content: 'first content', created_at: commonTimestamp },
      privkey,
    )
    ...
    for (;;) {
      event2 = await createEvent(
        { pubkey, kind: 10000, content: `second content ${nonce++}`, created_at: commonTimestamp },
        privkey,
      )
...
Then(
  /(\w+) receives (\d+) replaceable_event_0 event from (\w+) matching the lower ID event and EOSE/,
