...
When(
  /^(\w+) sends a parameterized_replaceable_event_1 event with content "([^"]+)" and tag (\w) containing "([^"]+)"$/,
  async function (name: string, content: string, tag: string, value: string) {
    const ws = this.parameters.clients[name] as WebSocket
    const { pubkey, privkey } = this.parameters.identities[name]
...
When(
  /^(\w+) sends a parameterized_replaceable_event_1 event with content "([^"]+)" and tag (\w) containing "([^"]+)" and expiring in the future$/,
  async function (name: string, content: string, tag: string, value: string) {
    const ws = this.parameters.clients[name] as WebSocket
    const { pubkey, privkey } = this.parameters.identities[name]
    ...
    const event: Event = await createEvent(
      {
        ...
        tags: [
          [tag, value],
          [EventTags.Expiration, Math.floor(new Date().getTime() / 1000 + 10).toString()],
        ],
      },
      privkey,
    )
...
Then(
  ...
  async function (name: string, count: string, author: string, content: string) {
    const ws = this.parameters.clients[name] as WebSocket
    const subscription = this.parameters.subscriptions[name][this.parameters.subscriptions[name].length - 1]
...
When(
  /^(\w+) subscribes to parameterized_replaceable_event_1 events from (\w+)$/,
  async function (this: World<Record<string, any>>, name: string, author: string) {
    ...
    const subscription = {
      ...
      filters: [{ kinds: [EventKinds.PARAMETERIZED_REPLACEABLE_FIRST + 1], authors: [authorPubkey] }],
    }
...
Then(
  /^(\w+) drafts a parameterized_replaceable_event_2 event with content "([^"]+?)" and tag (\w+) containing "([^"]+?)"$/,
  async function (name: string, content: string, tagName: string, tagValue: string) {
    const { pubkey, privkey } = this.parameters.identities[name]
